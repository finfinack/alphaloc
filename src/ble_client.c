...
static int cccd_read_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
                        struct ble_gatt_attr *attr, void *arg);
typedef struct
{
  const char *label;
...
static bool peer_is_bonded(const ble_addr_t *addr) {
  ...
  int num = 0;
  if (ble_store_util_bonded_peers(peers, &num, MYNEWT_VAL(BLE_STORE_MAX_BONDS)) != 0) {
    return false;
  }
  for (int i = 0; i < num; ++i) {
...
static bool name_prefix_match(const struct ble_hs_adv_fields *fields, const char *prefix)
{
  if (prefix == NULL || prefix[0] == '\0')
  {
    return true;
...
static void make_sony_uuid(uint16_t first, uint16_t second, ble_uuid128_t *out)
{
  uint8_t bytes[16] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                       0xFF, 0xFF, (uint8_t)(second & 0xFF), (uint8_t)(second >> 8),
                       (uint8_t)(first & 0xFF), (uint8_t)(first >> 8), 0x00, 0x80};
  memcpy(out->value, bytes, sizeof(bytes));
}

static bool is_sony_camera_adv(const struct ble_gap_disc_desc *desc, const struct ble_hs_adv_fields *fields)
{
  if (fields->mfg_data_len < sizeof(SONY_MFG_PREFIX))
  {
    return false;
  }
  uint16_t company_id = (uint16_t)fields->mfg_data[1] << 8 | fields->mfg_data[0];
  uint16_t product_id = (uint16_t)fields->mfg_data[3] << 8 | fields->mfg_data[2];
  ...
  if (s_cfg->camera_name_prefix[0] != '\0')
  {
    if (!name_prefix_match(fields, s_cfg->camera_name_prefix))
    {
      ...
      {
        char name_buf[32];
        size_t copy_len = fields->name_len < sizeof(name_buf) - 1 ? fields->name_len : sizeof(name_buf) - 1;
...
static int gatt_disc_chrs_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
...
static int gatt_disc_all_svc_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
                                const struct ble_gatt_svc *svc, void *arg);
static bool uuid_matches(const ble_uuid_any_t *uuid, uint16_t first, uint16_t second);
...
static int dd21_read_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
                        struct ble_gatt_attr *attr, void *arg)
{
  (void)conn_handle;
  (void)arg;
  ...
  s_dd21_ready = true;
  ESP_LOGI(TAG, "DD21 flag byte=0x%02X require_tz_dst=%d", buf[4], s_require_tz_dst);
...
static void try_start_ff02_dsc(uint16_t conn_handle)
{
  ...
  uint16_t end = s_handles.end_ff02 ? s_handles.end_ff02 : (s_handles.rem_svc_end ? s_handles.rem_svc_end : 0xFFFF);
...
static void dsc_retry_cb(void *arg)
{
  ...
  {
    if (s_dsc_pending_ff02 && s_handles.chr_ff02 != 0 && s_handles.cccd_ff02 == 0)
    {
      s_dsc_pending_ff02 = false;
      s_handles.cccd_ff02 = (uint16_t)(s_handles.chr_ff02 + 1);
      ESP_LOGW(TAG, "FF02 CCCD not found; using fallback handle=%u", s_handles.cccd_ff02);
...
static void enable_location_updates(uint16_t conn_handle)
{
  ...
  if (s_handles.chr_dd30 != 0)
  {
    ...
    ble_gattc_write_flat(conn_handle, s_handles.chr_dd30, &on, sizeof(on), NULL, NULL);
  }
  if (s_handles.chr_dd31 != 0)
  {
    uint8_t on = 0x01;
    ESP_LOGI(TAG, "Enabling location updates");
    ble_gattc_write_flat(conn_handle, s_handles.chr_dd31, &on, sizeof(on), NULL, NULL);
  }
  s_location_enabled = (s_handles.chr_dd30 != 0 && s_handles.chr_dd31 != 0);
  ESP_LOGI(TAG, "Location updates enabled");
}

static int gatt_disc_svc_cb(uint16_t conn_handle, const struct ble_gatt_error *error, const struct ble_gatt_svc *svc, void *arg)
{
  if (error->status == 0 && svc != NULL)
  {
    if (s_disc_state == DISC_LOC_SVC)
    {
      if (s_handles.loc_svc_start == 0)
      {
        ...
        VLOGI("Location service found: start=%u end=%u", svc->start_handle, svc->end_handle);
      }
    }
    else if (s_disc_state == DISC_REM_SVC)
    {
      if (s_handles.rem_svc_start == 0)
      {
        ...
        VLOGI("Remote service found: start=%u end=%u", svc->start_handle, svc->end_handle);
      }
    }
    return 0;
  }
  ...
  if (error->status == BLE_HS_EDONE)
  {
    if (s_disc_state == DISC_LOC_SVC && s_handles.loc_svc_start != 0)
    {
      ...
      return ble_gattc_disc_all_chrs(conn_handle, s_handles.loc_svc_start,
                                     s_handles.loc_svc_end, gatt_disc_chrs_cb, NULL);
    }
    if (s_disc_state == DISC_LOC_SVC && s_handles.loc_svc_start == 0)
    {
    ...
    if (s_disc_state == DISC_REM_SVC && s_handles.rem_svc_start != 0)
    {
      ...
      return ble_gattc_disc_all_chrs(conn_handle, s_handles.rem_svc_start,
                                     s_handles.rem_svc_end, gatt_disc_chrs_cb, NULL);
    }
    if (s_disc_state == DISC_REM_SVC && s_handles.rem_svc_start == 0)
    {
                                ...
                                const struct ble_gatt_svc *svc, void *arg)
{
  if (error->status == 0 && svc != NULL)
  {
    if ((uuid_matches(&svc->uuid, 0xDD00, 0xDD00) || uuid16_matches(&svc->uuid, 0xDD00)) &&
        s_handles.loc_svc_start == 0)
    {
      s_handles.loc_svc_start = svc->start_handle;
      s_handles.loc_svc_end = svc->end_handle;
      VLOGI("Location service found in all-svc: start=%u end=%u", svc->start_handle, svc->end_handle);
    }
    else if ((uuid_matches(&svc->uuid, 0xFF00, 0xFF00) || uuid16_matches(&svc->uuid, 0xFF00)) &&
             s_handles.rem_svc_start == 0)
    {
      s_handles.rem_svc_start = svc->start_handle;
      s_handles.rem_svc_end = svc->end_handle;
      VLOGI("Remote service found in all-svc: start=%u end=%u", svc->start_handle, svc->end_handle);
    }
    return 0;
  }
                             ...
                             const struct ble_gatt_chr *chr, void *arg)
{
  ...
  {
    if (s_last_chr_interest == 1 && s_handles.end_ff02 == 0 && chr->def_handle > 0)
    {
    ...
    {
      if (uuid_matches(&chr->uuid, 0xDD11, 0xDD00) || uuid16_matches(&chr->uuid, 0xDD11))
      {
        s_handles.chr_dd11 = chr->val_handle;
        VLOGI("Found DD11 handle=%u", chr->val_handle);
      }
      else if (uuid_matches(&chr->uuid, 0xDD21, 0xDD00) || uuid16_matches(&chr->uuid, 0xDD21))
      {
        s_handles.chr_dd21 = chr->val_handle;
        VLOGI("Found DD21 handle=%u", chr->val_handle);
      }
      else if (uuid_matches(&chr->uuid, 0xDD30, 0xDD00) || uuid16_matches(&chr->uuid, 0xDD30))
      {
        s_handles.chr_dd30 = chr->val_handle;
        VLOGI("Found DD30 handle=%u", chr->val_handle);
      }
      else if (uuid_matches(&chr->uuid, 0xDD31, 0xDD00) || uuid16_matches(&chr->uuid, 0xDD31))
      {
    ...
    {
      if (uuid_matches(&chr->uuid, 0xFF02, 0xFF00) || uuid16_matches(&chr->uuid, 0xFF02))
      {
        s_handles.chr_ff02 = chr->val_handle;
        VLOGI("Found FF02 handle=%u props=0x%02X (remote svc)", chr->val_handle, chr->properties);
  ...
  if (error->status == BLE_HS_EDONE)
  {
    ...
    {
      s_handles.end_ff02 = s_handles.rem_svc_end ? s_handles.rem_svc_end : 0xFFFF;
    }
    s_last_chr_interest = 0;
    ...
    if (s_disc_state == DISC_REM_CHR)
    {
      if (s_handles.chr_ff02 != 0)
      {
        ...
        return ble_gattc_disc_all_dscs(conn_handle, s_handles.rem_svc_start,
                                       s_handles.rem_svc_end, gatt_disc_dsc_cb, NULL);
      }
  ...
  if (error->status != BLE_HS_EDONE)
  {
    VLOGW("Characteristic discovery error=%d state=%d", error->status, s_disc_state);
  }
  return error->status;
}
                            ...
                            uint16_t chr_val_handle, const struct ble_gatt_dsc *dsc, void *arg)
{
  ...
  {
    if (dsc->uuid.u.type == BLE_UUID_TYPE_16 && dsc->uuid.u16.value == BLE_GATT_DSC_CLT_CFG_UUID16)
    {
                                   ...
                                   uint8_t *out, size_t *out_len)
{
  ...
  const bool send_tz_dst = require_tz_dst || (tz_off_min > 0 || dst_off_min > 0);
...
bool ble_client_send_location(const gps_fix_t *fix)
{
  ...
  if (!s_location_enabled)
  {
    ...
    else if (s_encrypted && s_handles.chr_dd21 != 0)
    {
      ...
      if (now - s_last_dd21_attempt_us > 3000000)
      {
        ...
        ble_gattc_read(s_handles.conn_handle, s_handles.chr_dd21, dd21_read_cb, NULL);
      }
    }
    return false;
  }
  ...
  if (!build_location_payload(fix, s_require_tz_dst, s_tz_off_min, s_dst_off_min,
                              payload, &payload_len))
  {
  ...
  if (payload_len > max_payload)
  {
    ...
    rc = ble_gattc_write_long(s_handles.conn_handle, s_handles.chr_dd11, 0, om, NULL, NULL);
  }
  else
  {
...
#endif
  VLOGI("Location write %s (%u bytes)", rc == 0 ? "ok" : "failed", (unsigned)payload_len);
...
bool ble_client_is_bonded(void)
{
  return s_bonded_camera;
}
...
int ble_client_gap_event_cb(struct ble_gap_event *event, void *arg)
{
  ...
  case BLE_GAP_EVENT_DISC:
  {
    ...
    ble_gap_connect(s_own_addr_type, &event->disc.addr, 30000, NULL, ble_client_gap_event_cb, NULL);
  ...
  case BLE_GAP_EVENT_NOTIFY_RX:
  {
    ...
    if (event->notify_rx.attr_handle == s_handles.chr_ff02)
    {
      ...
      if (event->notify_rx.om->om_len == sizeof(focus_msg) &&
          memcmp(event->notify_rx.om->om_data, focus_msg, sizeof(focus_msg)) == 0)
      {
  ...
  case BLE_GAP_EVENT_ENC_CHANGE:
    ...
    if (!s_encrypted && enc_failure_needs_rebond(event->enc_change.status))
    {
      ...
      {
        ble_store_util_delete_peer(&desc.peer_ota_addr);
        ESP_LOGW(TAG, "Bond mismatch suspected; deleting bond and retrying pairing");
      }
    ...
    if (s_encrypted)
    {
      ...
      if (!s_retried_disc_after_enc && s_handles.loc_svc_start == 0 && s_handles.rem_svc_start == 0)
      {
    ...
    if (s_encrypted && s_dd21_pending && s_handles.chr_dd21 != 0 && !s_dd21_ready)
    {
      s_dd21_pending = false;
      ble_gattc_read(s_handles.conn_handle, s_handles.chr_dd21, dd21_read_cb, NULL);
    }
...
static void ble_start_scan(void)
{
  ...
  ble_gap_disc(s_own_addr_type, BLE_HS_FOREVER, &params, ble_client_gap_event_cb, NULL);
