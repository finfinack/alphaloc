...
void config_set_defaults(app_config_t *cfg)
{
  ...
  strncpy(cfg->camera_name_prefix, "SonyA7", sizeof(cfg->camera_name_prefix) - 1);
...
static void config_read_str(nvs_handle_t nvs, const char *key, char *out, size_t out_len)
{
  size_t len = out_len;
...
bool config_load(app_config_t *cfg)
{
  ...
  config_read_str(nvs, "cam_name", cfg->camera_name_prefix, sizeof(cfg->camera_name_prefix));
  config_read_str(nvs, "cam_mac", cfg->camera_mac_prefix, sizeof(cfg->camera_mac_prefix));
